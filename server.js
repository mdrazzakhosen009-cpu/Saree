...
const defaults = {
 store_name: 'SAREE', logo: '/assets/logo-saree.png', delivery_promise: 'ঢাকার ভিতরে 1–2 দিন, ঢাকার বাইরে 2–4 দিন',
 opening_hours: 'প্রতিদিন সকাল 10টা থেকে রাত 10টা', store_info: 'SAREE-তে premium saree collection পাওয়া যায়—Katan, Jamdani, Organza, Cotton ও festive saree।',
 bkash_number: '01812-345678', nagad_number: '', rocket_number: '', payment_note: 'Send Money করে Transaction ID দিন।',
 bkash_enabled: '1', nagad_enabled: '1', rocket_enabled: '1', cod_enabled: '1',
 admin_password_hash: crypto.createHash('sha256').update('admin123').digest('hex'), ...quickDefaults
};
...
app.get('/api/products', (req, res) => {
  ...
  let sql = 'SELECT * FROM products WHERE 1=1', a = [];
  if (category && category !== 'all') { sql += ' AND category=?'; a.push(category); }
  if (search) {
    sql += ' AND (name LIKE ? OR tags LIKE ? OR category LIKE ? OR description LIKE ?)';
    const q = '%' + search + '%';
    a.push(q, q, q, q);
  }
...
app.post('/api/orders', (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO orders(customer_name,phone,address,payment_method,payment_number,transaction_id,total,status,items_json) VALUES(?,?,?,?,?,?,?,?,?)`)
    .run(customer_name, phone, address, payment_method, payNum, transaction_id || '', total, 'Pending', JSON.stringify(cleanItems));
...
app.get('/api/orders/track/:id', (req, res) => {
  const raw = String(req.params.id).replace(/^SAR-/i, '');
  const o = db.prepare('SELECT id,status,total,created_at FROM orders WHERE id=?').get(Number(raw));
  if (!o) return res.status(404).json({ error: 'Order not found' });
  res.json({ ...o, public_id: publicId(o.id) });
});

app.post('/api/admin/login', (req, res) => {
  const s = getSettings(), hash = crypto.createHash('sha256').update(String(req.body.password || '')).digest('hex');
  if (hash !== s.admin_password_hash) return res.status(401).json({ error: 'Wrong password' });
...
app.get('/api/admin/dashboard', auth, (req, res) => {
  ...
  res.json({
    ...
    agents: db.prepare('SELECT COUNT(*) c FROM agents WHERE active=1').get().c
  });
});
...
app.post('/api/admin/products', auth, upload.single('image'), (req, res) => {
  ...
  const r = db.prepare('INSERT INTO products(name,price,old_price,category,description,tags,image,featured,is_new) VALUES(?,?,?,?,?,?,?,?,?)')
    .run(b.name, Number(b.price), Number(b.old_price || 0), b.category || 'Saree', b.description || '', b.tags || '', image, bool(b.featured) ? 1 : 0, bool(b.is_new) ? 1 : 0);
...
app.put('/api/admin/products/:id', auth, upload.single('image'), (req, res) => {
  ...
  const b = req.body, image = req.file ? '/uploads/' + req.file.filename : (b.image_url || old.image);
  db.prepare('UPDATE products SET name=?,price=?,old_price=?,category=?,description=?,tags=?,image=?,featured=?,is_new=? WHERE id=?')
    .run(b.name, Number(b.price), Number(b.old_price || 0), b.category || 'Saree', b.description || '', b.tags || '', image, bool(b.featured) ? 1 : 0, bool(b.is_new) ? 1 : 0, req.params.id);
  res.json({ ok: true });
});
...
app.post('/api/admin/agents', auth, (req, res) => {
  const b = req.body, r = db.prepare('INSERT INTO agents(name,whatsapp,messenger_url,active) VALUES(?,?,?,?)').run(b.name, b.whatsapp, b.messenger_url || '', bool(b.active) ? 1 : 0);
  res.json({ ok: true, id: r.lastInsertRowid });
});
app.put('/api/admin/agents/:id', auth, (req, res) => {
  const b = req.body;
  db.prepare('UPDATE agents SET name=?,whatsapp=?,messenger_url=?,active=? WHERE id=?').run(b.name, b.whatsapp, b.messenger_url || '', bool(b.active) ? 1 : 0, req.params.id);
  res.json({ ok: true });
});
app.delete('/api/admin/agents/:id', auth, (req, res) => {
...
app.put('/api/admin/settings', auth, (req, res) => {
  ...
  for (const [k, v] of Object.entries(b)) {
    ...
    db.prepare('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run(k, String(bool(v) ? '1' : v));
  }
...
app.post('/api/admin/ai-product', auth, upload.single('image'), async (req, res) => {
  ...
  let preview = {
    ...
    description: req.body.description || 'Premium saree with elegant finishing, suitable for festive and special occasions.',
    tags: req.body.tags || 'saree,premium,festive',
    image
  };
  if (process.env.OPENAI_API_KEY && req.file) {
    try {
      ...
      console.error('AI product import:', e.message);
    }
  }
  res.json({ preview });
});

app.post('/api/admin/ai-setup-image', auth, upload.single('image'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Image required' });
  const url = '/uploads/' + req.file.filename;
  db.prepare('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('ai_proof_image', url);
  res.json({ ok: true, url, settings: getSettings() });
});
...
function catalogText(products) {
  return products.map(p => `ID ${p.id}: ${p.name} | ৳${p.price} | old ৳${p.old_price || 0} | category ${p.category} | tags ${p.tags} | ${p.description}`).join('\n');
}
...
app.post('/api/chat', async (req, res) => {
  ...
  if (greeting(msg)) {
    return res.json({
      reply: `আসসালামু আলাইকুম ❤️ SAREE-তে আপনাকে স্বাগতম। আমি SAREE-এর shopping assistant। শাড়ি দেখা, availability, price, delivery বা সরাসরি order করতে আমাকে বলুন।`,
      quick
    });
  }
  ...
  for (const [q, a] of qMap) {
    if (q && msg.toLowerCase() === String(q).toLowerCase()) {
      return res.json({
        reply: a,
        proof_image: msg.toLowerCase() === String(s.ai_q5_title).toLowerCase() ? s.ai_proof_image : '',
        quick
      });
    }
  }
  ...
  const payments = activePayments(s).map(x => x.method + ' ' + x.number).join(', ');
  ...
  try {
    ...
    if (ai) {
      if (ai.product_id) {
        const p = products.find(x => x.id === Number(ai.product_id));
        if (p) {
          state.order = state.order || {};
          state.order.product_id = p.id;
          state.order.quantity = Math.max(1, Number(ai.quantity || state.order.quantity || 1));
        }
      }
      for (const k of ['name', 'phone', 'address', 'payment_method', 'transaction_id']) {
        if (ai[k]) {
          state.order = state.order || {};
          state.order[k] = ai[k];
        }
      ...
      if (ai.intent === 'order' || state.order) {
        ...
        else if (p && o.payment_method && o.payment_method !== 'Cash on Delivery' &
